...
def main() -> None:
    ...
    if existing_draft is not None:
        ...
        upsert_crm_account(
            customer,
            status="ready_to_send" if decision == "send_now" else "scheduled",
            next_action="send_sales_email" if decision == "send_now" else "wait_until_scheduled_send",
    ...
    upsert_crm_account(
        customer,
        status="ready_to_send" if earliest_send_at <= now else "scheduled",
        next_action="send_sales_email" if earliest_send_at <= now else "wait_until_scheduled_send",
        last_subject=str(draft.get("subject") or ""),
        recommended_offer=str(plan.get("primary_offer") or ""),
        likely_pain=str((plan.get("customer_brief") or {}).get("pain_point") or ""),
    )
    log_agent(
