...
def main() -> None:
    ...
    if not customer.get("email"):
        ...
        reasons.append("Target account record does not have an email address.")
    ...
    if customer.get("opt_in_status") in {"unsubscribed", "opted_out"}:
        ...
        reasons.append("Recipient is opted out from email outreach.")

    plan["policy_decision"] = {
        "decision": "block" if violated_rules else "allow",
        "reasons": reasons if violated_rules else ["Recipient is active and subscribed."],
