...
AGENT_ID = "account_research_agent"
logger = logging.getLogger("mn.blueprint.gtm_ai.account_research")
...
def main() -> None:
    ...
    if existing_draft is not None:
        ...
        upsert_crm_account(
            customer,
            status="ready_draft_found",
            next_action="review_or_send_ready_outreach",
            last_subject=str(existing_draft.get("subject") or ""),
        )
    ...
    system_prompt = (
        "You are the account research agent in a multi-agent B2B GTM AI workflow. "
        "Return compact JSON only. Build a strategic brief for one high-quality sales email. "
        "Start from supplied account signals, infer likely pain cautiously, and never claim unverified facts. "
        "Every brief must include at least two GTM triggers: market signal, likely pain, business outcome, or ease. "
        ...
        "If campaign_type is response_followup or reply_followup, write the brief as if a helpful human is replying in-thread."
    )
    user_prompt = json.dumps(
        {
            ...
            "campaign_playbook": playbooks.get(plan.get("campaign_type", "gtm_workflow_spotlight"), {}),
            "offers_catalog": offers_catalog,
            "recent_email_memory": latest_source,
            "task": "Create a concise but strategy-rich account brief for personalized B2B sales outreach.",
        },
        indent=2,
    )
    ...
    likely_pain = str(brief.get("pain_point") or "")
    next_action = "draft_personalized_outreach"
    upsert_crm_account(
        customer,
        status="researched",
        next_action=next_action,
        likely_pain=likely_pain,
        recommended_offer=str(brief.get("recommended_offer") or plan.get("primary_offer") or ""),
        insight_tags=[plan.get("audience_segment", ""), plan.get("campaign_type", "")],
    )
    append_market_insight(
        customer,
        pain_signal=likely_pain,
        positioning_insight=str(brief.get("customer_angle") or brief.get("offer_reason") or ""),
        recommended_product_feedback="Review repeated pain signals for product roadmap and onboarding proof.",
        recommended_marketing_feedback="Use the account's exact pain language to sharpen positioning and outbound copy.",
        source="account_research_agent",
    )
    log_agent(
        runtime_job_id,
        AGENT_ID,
        "Prepared account research brief.",
        details={
