...
logger = logging.getLogger("mn.blueprint.gtm_ai.inbox_reply")
...
try:
    from gtm_outreach_skill import build_design_slots, render_email_html
...
def save_sent_email_copy(
    *,
    ...
    metadata = {
        ...
        "has_card_email_marker": "GTM signals to explore" in str(html_body or ""),
        "has_personal_reply_marker": (
            "data-slot=\"body_section\"" in str(html_body or "")
            and "GTM signals to explore" not in str(html_body or "")
        ),
    }
...
def render_reply_html(*, subject: str, reply_text: str, inbound_body: str = "") -> str:
    ...
    plan = {
        ...
        "primary_offer": "GTM AI Workflow",
        "goal": "Reply to an inbound GTM conversation",
        ...
        "draft": {
            "subject": f"Re: {subject}",
            "preview_text": "A quick personal reply from GTM AI Workflow.",
            "eyebrow": "Thanks for writing",
            "headline": "A quick note back on the GTM workflow",
            "body_sections": body_sections,
            "cta_label": "Review the GTM workflow",
            "cta_url_slug": "signal-loop-assessment",
            "secondary_text": "You can reply here with any other details and we will keep the CRM and next action updated.",
...
def send_slack_reply_report(*, to_email: str, subject: str, delivery: dict) -> dict:
    ...
    message = (
        f"GTM AI inbox reply report: reply to <{to_email}> was {status}. "
...
def crm_output_dir() -> Path:
    configured = (
        os.environ.get("GTM_CRM_CSV_DIR", "").strip()
        or os.environ.get("MN_CRM_CSV_DIR", "").strip()
        or os.environ.get("SYNAPTIC_CRM_CSV_DIR", "").strip()
...
def _read_crm_rows(path: Path) -> list[dict[str, str]]:
    if not path.exists():
        return []
...
def _write_crm_rows(path: Path, rows: list[dict[str, str]]) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=CRM_FIELDS, extrasaction="ignore")
        writer.writeheader()
        for row in rows:
            writer.writerow({field: row.get(field, "") for field in CRM_FIELDS})


def _summarize_reply(body: str) -> str:
    compact = " ".join(str(body or "").split())
    return compact[:240]


def upsert_crm_reply(*, from_email: str, subject: str, body: str, customer_id: str) -> None:
    path = crm_output_dir() / "crm.csv"
    rows = _read_crm_rows(path)
    now = utc_now()
    idx = next(
        (
            pos
            for pos, row in enumerate(rows)
            if row.get("account_id") == customer_id
            or row.get("email", "").lower() == from_email.lower()
        ),
        None,
    )
    existing = rows[idx] if idx is not None else {}
    response_summary = _summarize_reply(body)
    row = {
        **existing,
        "account_id": existing.get("account_id") or customer_id,
        "email": existing.get("email") or from_email,
        "source": existing.get("source") or "inbound_reply",
        "stage": existing.get("stage") or "response",
        "status": "response_received",
        "last_touch_at": now,
        "last_subject": subject,
        "last_outreach_status": existing.get("last_outreach_status") or "sent",
        "response_summary": response_summary,
        "next_action": "summarize_response_and_prepare_sales_followup",
        "insight_tags": "response_followup",
    }
    if idx is None:
        rows.append(row)
    else:
        rows[idx] = row
    _write_crm_rows(path, rows)

    insight_path = crm_output_dir() / "market_insights.csv"
    exists = insight_path.exists()
    with insight_path.open("a", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            {
                ...
                "title": row.get("title", ""),
                "pain_signal": row.get("likely_pain", ""),
                "response_signal": response_summary,
                "positioning_insight": "Use the reply language to refine pain hypotheses and outreach positioning.",
                "recommended_product_feedback": "Review whether the reply reveals missing integration, workflow, or trust requirements.",
                "recommended_marketing_feedback": "Feed objection and phrasing back into product marketing copy.",
                "source": "inbox_reply_agent",
            }
        )


def log_customer_reply(from_email: str, body: str, subject: str = "Inbound GTM reply"):
    db_path = os.environ.get("SYNAPTIC_DB_PATH", "/tmp/mn_gtm_ai_workflow.db")
    if not os.path.exists(db_path):
        upsert_crm_reply(
            from_email=from_email,
            subject=subject,
            body=body,
            customer_id=f"reply_{safe_artifact_slug(from_email, 'unknown')}",
        )
        return
    ...
    try:
        ...
        row = conn.execute(
            "SELECT customer_id FROM email_drafts WHERE status = 'sent' ORDER BY sent_at DESC LIMIT 1"
        ).fetchone()
        customer_id = row["customer_id"] if row else f"reply_{safe_artifact_slug(from_email, 'unknown')}"
...
def generate_reply_via_llm(body_content):
    try:
        ...
        return completion_text(
            "You are a concise, helpful B2B sales teammate for GTM AI Workflow. "
            "Summarize the buyer's signal, acknowledge their point, and offer one practical next step. "
            "Keep the response friendly but concise (1-3 sentences max).",
...
def check_agentmail() -> list:
    ...
    try:
        ...
        for msg in messages:
            ...
            log_customer_reply(from_email, body, subject)
