...
def infer_audience_segment(
    ...
    if any(token in job_title for token in ["revops", "sales ops", "operations", "crm"]):
        return "sales_ops"
    if any(token in job_title for token in ["product marketing", "positioning", "pmm"]):
        return "product_marketing"
    if any(token in job_title for token in ["founder", "ceo", "co-founder"]):
        return "founders"
    if any(token in job_title for token in ["revenue", "sales", "growth", "cro"]):
        return "revenue_leaders"
    if any(token in tags for token in ["revops", "crm", "sales-ops"]):
        return "sales_ops"
    if any(token in tags for token in ["product-marketing", "positioning", "launch"]):
        return "product_marketing"
    if any(token in tags for token in ["founder", "gtm", "early-pipeline"]):
        return "founders"
    return "revenue_leaders"
...
def infer_campaign_type(recent_activities: list[dict[str, Any]]) -> str:
    ...
    if any(token in summaries for token in ["product marketing", "positioning", "objection"]):
        return "product_marketing_feedback"
    if any(token in summaries for token in ["research signal", "target account", "icp"]):
        return "target_account_discovery"
    if any(token in summaries for token in ["likely pain", "crm", "pipeline", "reply summaries"]):
        return "pain_point_outreach"
    if any(token in summaries for token in ["clicked", "workflow", "guide", "pricing"]):
        return "pain_hypothesis_followup"
    return "pain_point_outreach"
...
def choose_primary_offer(
    ...
    if campaign_type in {"target_account_discovery", "pain_point_outreach"}:
        offer = find_offer_by_keywords("account", "outreach", "pilot")
        if offer:
            return offer

    if campaign_type == "product_marketing_feedback":
        offer = find_offer_by_keywords("signal", "feedback", "positioning")
        if offer:
            return offer

    if campaign_type in {"reply_followup", "response_followup"}:
        offer = find_offer_by_keywords("crm", "learning", "loop")
        if offer:
            return offer

    if "crm" in summaries or "revops" in summaries:
        offer = find_offer_by_keywords("crm", "learning", "loop")
        if offer:
            return offer

    if "positioning" in summaries or "product marketing" in summaries:
        offer = find_offer_by_keywords("signal", "positioning", "feedback")
        if offer:
            return offer
...
def build_customer_brief(
    *,
    ...
    strategy = {
        "campaign_type": plan.get("campaign_type", "gtm_workflow_spotlight"),
    ...
    if not proof_points:
        proof_points = [
            "It connects the account signal to one clear next step.",
            "It keeps the outreach specific enough for a useful reply.",
        ]
    recent_template = (
    ...
    return {
        ...
        "customer_angle": (
            ...
            or f"{customer.get('name')} is a {customer.get('job_title') or customer.get('title') or 'target account'}"
            f" at {customer.get('company') or 'the account'} who may benefit from a clearer GTM signal loop."
        ),
        "job_to_be_done": segment.get(
            "job_to_be_done",
            "Understand the GTM value quickly and decide on a useful next step.",
        ),
        "pain_point": segment.get(
            "pain_point",
            "Needs a relevant GTM recommendation without extra complexity or noise.",
        ),
        "recommended_offer": offer_name,
        "offer_reason": playbook.get(
            "offer_reason",
            "It matches the account signal and gives the recipient one simple decision to make.",
        ),
        ...
        "story_prompt_example": offer.get(
            ...
            "Run one account through research, pain hypothesis, outreach, response summary, CRM update, and insight feedback.",
        ),
        "objection_to_address": segment.get(
            "objection_to_address",
            "Why is this relevant right now, and why is it worth a reply?",
        ),
        "primary_cta": {
        ...
        "recommended_template": playbook.get("recommended_template", "gtm_workflow_spotlight"),
...
def normalize_structured_draft(
    ...
    *,
    ...
    workflow_example = str(brief.get("story_prompt_example") or "").strip()
    ...
    likely_pain = str(brief.get("pain_point") or "").strip()
    account_signal = str(customer.get("profile") or brief.get("customer_angle") or "").strip()
    company = str(customer.get("company") or "your team").strip()
    if len(subject_candidates) < 2:
        if campaign_type in {"reply_followup", "response_followup"}:
            ...
            subject_candidates = [
                subject_angle or f"{first_name}, a practical GTM signal-loop idea",
                f"A small workflow idea for {company}",
                f"{offer} could fit this GTM motion",
            ]
    preview_text = str(
        ...
        or (
            ...
            if campaign_type in {"reply_followup", "response_followup"}
            else f"A practical GTM recommendation based on the account signal behind {offer}."
        )
    ).strip()
        ...
        if campaign_type in {"reply_followup", "response_followup"}:
            ...
            body_sections = [
                ...
                (
                    ...
                    if inbound_summary
                    else workflow_example
                    or "If useful, I can map the next action and show how the reply becomes CRM state plus a product or marketing insight."
                ),
            ]
        else:
            body_sections = [
                f"Hi {first_name}, I noticed a signal at {company} that looks like it may connect to {offer}.",
                (
                    proof_points[0]
                    if proof_points
                    else likely_pain
                    or "Teams often have useful GTM signal spread across account research, outreach, replies, and CRM notes."
                ),
                (
                    proof_points[1]
                    if len(proof_points) > 1
                        ...
                        brief.get("offer_reason")
                        or "A small pilot can show whether the workflow improves pipeline quality and positioning without touching production systems."
                    )
                ),
            ]
    cta_label = str(
    ...
    secondary_text = str(
        ...
        or (
            "If useful, just hit reply with the GTM workflow you want to tighten first."
            if strategy["campaign_type"] in {"reply_followup", "response_followup"}
            else brief.get("objection_to_address")
            or "If this is not the right moment, the same loop can start with a small test account list later."
        )
    ).strip()
    footer_variant = (
...
def review_email_quality(
    *,
    ...
    if plan.get("audience_segment") in {
        ...
        issues.append("missing_gtm_signal_context")
        score -= 10
...
def allows_personal_reply_template(plan: dict[str, Any]) -> bool:
    return plan.get("campaign_type") in {"reply_followup", "response_followup"} and has_thread_reply_context(plan)
...
def select_template_name(
    *,
    ...
    mapping = {
        "gtm_workflow_spotlight": "gtm_workflow_spotlight",
        "signal_loop_reminder": "signal_loop_reminder",
        "pain_hypothesis_followup": "pain_hypothesis_followup",
        "reply_followup": (
            "personal_reply" if allows_personal_reply_template(plan) else "gtm_workflow_spotlight"
        ),
        "response_followup": (
            "personal_reply" if allows_personal_reply_template(plan) else "gtm_workflow_spotlight"
        ),
        "target_account_discovery": "signal_to_outreach",
        "pain_point_outreach": "pain_hypothesis_followup",
        "sales_email_prepare": "gtm_workflow_spotlight",
        "product_marketing_feedback": "conversation_signal_reveal",
    }
    mapped = mapping.get(plan.get("campaign_type", "gtm_workflow_spotlight"))
    if mapped in template_library:
        return str(mapped)
    return next(iter(template_library.keys()), "gtm_workflow_spotlight")
