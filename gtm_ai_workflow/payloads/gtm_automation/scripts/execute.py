...
def save_sent_email_copy(
    *,
    ...
    metadata = {
        ...
        "has_card_email_marker": "GTM signals to explore" in html_body,
        ...
        "has_personal_reply_marker": (
            ...
            and "GTM signals to explore" not in html_body
        ),
    }
...
def format_round_slack_report(
    *, round_stats: dict, customer: dict, actual_recipient: str, status: str
    ...
    return (
        f"GTM AI workflow round {round_stats['cycle']} report: "
...
def html_matches_design_template(html_body: str, design_template: str) -> bool:
    ...
    if design_template == "card_email.html":
        return (
            "GTM signals to explore" in html_body
...
def normalize_delivery_render_plan(render_plan: dict) -> dict:
    ...
    normalized["campaign_type"] = "gtm_workflow_spotlight"
...
def main(email_sender=None, slack_sender=None) -> None:
    ...
    test_recipient = (
        os.environ.get("GTM_TEST_EMAIL_TO", "").strip()
        or os.environ.get("SYNAPTIC_TEST_EMAIL_TO", "").strip()
    ...
    if policy_decision.get("decision") == "block":
        ...
        upsert_crm_account(
            customer,
            status="blocked",
            next_action="review_outreach_policy",
            last_subject=str(saved_draft.get("subject") or ""),
            last_outreach_status="blocked",
            likely_pain=str((plan.get("customer_brief") or {}).get("pain_point") or ""),
            recommended_offer=str(plan.get("primary_offer") or ""),
        )
        ...
        upsert_crm_account(
            customer,
            status="scheduled",
            next_action="wait_until_scheduled_send",
            last_subject=str(saved_draft.get("subject") or ""),
            last_outreach_status="waiting",
            likely_pain=str((plan.get("customer_brief") or {}).get("pain_point") or ""),
            recommended_offer=str(plan.get("primary_offer") or ""),
        )
    else:
        ...
        if delivery["status"] == "sent":
            ...
            upsert_crm_account(
                ...
                next_action="monitor_response",
                last_subject=str(saved_draft.get("subject") or ""),
                last_outreach_status="sent",
                last_delivery_recipient=actual_recipient,
                test_recipient_override=bool(test_recipient),
                likely_pain=str((plan.get("customer_brief") or {}).get("pain_point") or ""),
                recommended_offer=str(plan.get("primary_offer") or ""),
                insight_tags=[plan.get("campaign_type", ""), "outreach_sent"],
            )
            append_market_insight(
                customer,
                pain_signal=str((plan.get("customer_brief") or {}).get("pain_point") or ""),
                positioning_insight=str((plan.get("customer_brief") or {}).get("offer_reason") or ""),
                recommended_product_feedback="Watch replies for unmet workflow requirements and integration objections.",
                recommended_marketing_feedback="Compare reply language against the pain hypothesis used in this outreach.",
                source="gtm_automation_agent",
            )
            log_agent(
                runtime_job_id,
                AGENT_ID,
            ...
            upsert_crm_account(
                ...
                next_action="inspect_delivery_error",
                last_subject=str(saved_draft.get("subject") or ""),
                last_outreach_status=str(delivery.get("status") or "failed"),
                last_delivery_recipient=actual_recipient,
                test_recipient_override=bool(test_recipient),
                likely_pain=str((plan.get("customer_brief") or {}).get("pain_point") or ""),
                recommended_offer=str(plan.get("primary_offer") or ""),
