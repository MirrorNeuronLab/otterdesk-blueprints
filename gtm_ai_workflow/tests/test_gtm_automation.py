...
EXECUTE_PATH = BLUEPRINT_DIR / "payloads" / "gtm_automation" / "scripts" / "execute.py"


def load_execute_module():
    spec = importlib.util.spec_from_file_location("gtm_automation_execute", EXECUTE_PATH)
...
class GtmAutomationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.crm_dir = os.path.join(self.tmp.name, "crm")
        ...
        os.environ["GTM_TEST_EMAIL_TO"] = ""
        os.environ["GTM_CRM_CSV_DIR"] = self.crm_dir
    ...
    def write_plan(self):
        plan = {
            ...
            "cycle": 2,
            "campaign_type": "pain_point_outreach",
            "customer": {
                "customer_id": "cust_1",
                "name": "Avery",
            ...
            "primary_offer": "Local CRM learning loop",
            "customer_brief": {
                "pain_point": "CRM state and reply summaries drift away from outreach.",
                "offer_reason": "A local CSV loop makes the workflow easy to inspect.",
            },
            "control_decision": {"decision": "send_now"},
            "policy_decision": {"decision": "allow"},
            "saved_draft": {
                "draft_id": "draft_test",
                "subject": "A small GTM loop idea",
    ...
    def test_injected_email_sender_logs_sent_event(self):
        ...
        self.assertIn(
            {
                "type": "email_sent",
                "payload": {"to": "test@example.com", "subject": "A small GTM loop idea"},
            },
            payload["events"],
        )
        ...
        self.assertIn("outreach_sent", crm_text)
        self.assertIn("test@example.com", crm_text)
        self.assertIn("gtm_automation_agent", insights_text)
    ...
    def test_delivery_rerenders_old_default_html_with_card_template(self):
        ...
        payload = json.loads(output.getvalue())
        self.assertEqual(payload["events"][0]["payload"]["status"], "sent")
        self.assertIn("GTM signals to explore", sent_requests[0]["html"])
        ...
        plan["saved_draft"]["source_payload_json"] = json.dumps(
            {
                ...
                "draft": {
                    "subject": "Re: A small GTM loop idea",
    ...
    def test_loads_pending_ready_draft_when_runtime_message_lacks_saved_draft(self):
        ...
        event = payload["events"][0]["payload"]
        self.assertEqual(event["status"], "sent")
        self.assertEqual(event["subject"], "A small GTM loop idea")
