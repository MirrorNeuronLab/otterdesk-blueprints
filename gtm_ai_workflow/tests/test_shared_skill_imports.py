...
def load_account_research_execute_module():
    execute_path = PAYLOADS_DIR / "account_research" / "scripts" / "execute.py"
    spec = importlib.util.spec_from_file_location("account_research_execute", execute_path)
...
def test_payload_sitecustomize_exposes_shared_marketing_skill():
    env = os.environ.copy()
    env["PYTHONPATH"] = str(PAYLOADS_DIR / "account_research")
...
def test_runtime_get_plan_accepts_explicit_emit_body():
    env = os.environ.copy()
    env["PYTHONPATH"] = str(PAYLOADS_DIR / "gtm_automation")
    envelope = {
        "body": {
            "customer": {"customer_id": "rendered"},
            "saved_draft": {"draft_id": "draft_rendered"},
...
def test_host_local_uploads_shared_skills_with_each_executor():
    ...
    for worker in executor_workers:
        ...
        assert "_shared_skills/gtm_outreach_skill" in sources
...
def test_campaign_template_selection_keeps_gtm_outreach_on_card_design():
    env = os.environ.copy()
...
for campaign in ["target_account_discovery", "pain_point_outreach", "product_marketing_feedback", "gtm_workflow_spotlight"]:
...
def test_customer_reply_activity_alone_does_not_select_reply_followup():
    module = load_account_research_execute_module()
    activities = [{"summary": "Customer replied: This sounds interesting."}]

    assert (
        module.should_select_reply_followup(
            plan={"reply_context": {"subject": "Re: GTM signal loop", "text_body": "Interested"}},
...
def test_account_research_runs_with_blueprint_local_gtm_skill(tmp_path):
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(PAYLOADS_DIR / "account_research" / "scripts" / "execute.py"),
        ],
        cwd=BLUEPRINT_DIR,
        env=env,
        capture_output=True,
...
def test_fresh_runtime_db_is_seeded_from_input_data(tmp_path):
    ...
    assert counts["ready_drafts"] >= 1
    assert "GTM signals to explore" in counts["ready_html"]
