...
The workflow deliberately mixes execution modes. `frame_research_goal`, `retrieve_and_evaluate_evidence`, and `verify_and_publish_packet` are deterministic HostLocal stages. `autonomous_research` is the only OpenShell node and runs all autonomous subphases in one isolated, one-shot sandbox. Generated code never runs in a deterministic stage.
