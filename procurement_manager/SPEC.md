...
The optional structured `procurement_case.json` input is evaluated in a
separate branch after evidence preparation. Its decimal cash-flow and mandatory
fit checks join the existing recommendation audit at publication. The branch
publishes exact-digest review requests to the existing conversation controls.
Responses are recorded but are not yet joined into an approved case. The
branch does not wait for live supplier events or continue to mock ordering;
`READY_FOR_REVIEW` denotes a packet awaiting review, not an
approved purchase.
