"""Message-neutral binding for Procurement Manager specialists."""
...
def create_domain_agent(agent_id: str, operation: Callable[..., dict[str, Any]], *, additional_artifact: tuple[str, str] | None = None):
    def invoke(
        context: StatefulStepContext,
        *,
        ...
        **options: Any,
        ...
        artifacts = [ref]
        if additional_artifact is not None and result.get("case_packet"):
            artifact_id, artifact_path = additional_artifact
            artifacts.append(artifact_reference(artifact_id, artifact_path))
        payload: dict[str, Any] = {
            "result": {
                key: (artifacts[-1] if key == "case_packet" and additional_artifact is not None and value else value)
