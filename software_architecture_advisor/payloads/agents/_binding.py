...
def bind_child(operation):
    ...
    def invoke(context, *, agent_input, llm_client=None, **options):
        ...
        if not isinstance(work, dict) or "_child" not in work:
            raise ValueError("A child specialist requires a committed runtime input")
        try:
            payload = operation(context.to_mapping(), work, llm_client=llm_client)
        except Exception as exc:
            from domain.investigation_store import record_failure
            record_failure(context.to_mapping(), work, exc)
            raise
        return AgentHandlerOutput(payload=payload)
