"""Message-neutral binding for read-only architecture-advisor specialists."""
...
def create_domain_agent(agent_id: str, operation: Callable[..., dict[str, Any]]):
    def invoke(context: StatefulStepContext, *, agent_input: AgentInput, llm_client: Any | None = None, **options: Any) -> AgentHandlerOutput:
        result = operation(context.to_mapping(), llm_client=llm_client, **options)
        state_ref = artifact_reference("software_architecture_advisor_state", "workflow_state/software_architecture_advisor_state.json")
        artifacts = [state_ref]
        payload: dict[str, Any] = {"result": {key: value for key, value in result.items() if key not in {"final_artifact", "output_files"}}, "state_artifact": state_ref}
        ...
        return AgentHandlerOutput(payload=payload, artifacts=tuple(artifacts), metrics={"agent_id": agent_id, "read_only": True})
