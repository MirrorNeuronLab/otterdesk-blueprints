...
def map_architecture(
    ...
    component_fallback = {
        ...
        "entrypoints": fallback_entrypoints,
    ...
    component_map = run_model_stage(
        ...
        validator=lambda value: _validate_component_map(
            value,
            fact_ids=fact_ids,
            paths=paths,
            baseline_components=fallback_components,
            baseline_entrypoints=fallback_entrypoints,
        ),
...
def _validate_component_map(
    ...
    baseline_components: list[dict[str, Any]] | None = None,
    baseline_entrypoints: list[dict[str, Any]] | None = None,
) -> dict[str, Any] | None:
    if not isinstance(value, dict):
        return None
    components = []
    component_ids = set()
    raw_components = value.get("components") if isinstance(value.get("components"), list) else []
    for index, item in enumerate(raw_components[:30], start=1):
    ...
    if not components and baseline_components:
        components = [dict(item) for item in baseline_components]
    ...
    if not entrypoints and baseline_entrypoints:
        entrypoints = [dict(item) for item in baseline_entrypoints]
    directions = []
    for item in value.get("dependency_directions") or []:
        ...
        source = text(item.get("from_component"), maximum=300)
        target = text(item.get("to_component"), maximum=300)
        relationship = text(item.get("relationship"), maximum=1000)
        if (
            cited is None
            or cited_paths is None
    ...
    if not summary or not components:
        return None
    return {"summary": summary, "components": components, "entrypoints": entrypoints, "dependency_directions": directions[:40], "unknowns": unknowns or []}
...
def _validate_cross_cutting(
    ...
    for field in ("flows", "state_ownership", "trust_boundaries", "deployment_interactions", "test_observations"):
        ...
        for item in raw_items[:30]:
            ...
            confidence = text(item.get("confidence"), maximum=20).lower()
            if (
                cited is None
                or cited_paths is None
                or not name
