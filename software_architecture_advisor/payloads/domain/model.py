...
def response_schema(data):
    ...
    result = obj({"verdict": {"type": "string", "enum": ["supported", "contradicted", "inconclusive"]},
    ...
    if data.get("review_policy"):
        result["properties"]["action_kind"] = {"type": "string", "enum": ["verify", "preserve", "change"]}
        result["required"].append("action_kind")
        if data.get("prior_proposal", {}).get("missing_evidence"):
            result["properties"]["action_kind"]["enum"] = ["verify", "preserve"]
        if data.get("unavailable_evidence"):
            result["properties"]["verdict"]["enum"] = ["inconclusive"]
            result["properties"]["action_kind"]["enum"] = ["verify"]
    return result
