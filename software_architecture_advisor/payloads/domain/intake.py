...
from .model_analysis import build_source_packet, run_model_stage, string_list, text
    ...
    def validate(value: dict[str, Any]) -> dict[str, Any] | None:
        ...
        for item in priorities[:12]:
            if not isinstance(item, dict):
                continue
            raw_refs = item.get("path_refs")
            refs = (
                list(
                    dict.fromkeys(
                        str(ref).strip()
                        for ref in raw_refs
                        if str(ref).strip() in packet_paths
                    )
                )
                if isinstance(raw_refs, list)
                else []
            )
            targets = string_list(item.get("evidence_targets"), maximum_items=10) or []
        ...
        unknowns = string_list(value.get("unknowns"), maximum_items=20) or []
