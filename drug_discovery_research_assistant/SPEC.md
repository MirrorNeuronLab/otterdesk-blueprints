...
After ranking, the blueprint domain package renders an optional run-scoped `web/index.html` result page from workflow events plus `workflow_state/drug_discovery_state.json`, `service_state.json`, `cycle_progress.json`, `leading_candidate.json`, and `final_artifact.json`. It also writes a `static_html` `web_ui.json` handle for the runtime proxy. The page is not an agent, entrypoint, service, or health-checked dependency. Rendering occurs only after the authoritative report is durable and any rendering failure is contained, so web UI preparation or availability cannot prevent the scientific batch from starting or completing. The page exposes one leading candidate's identifier, SMILES string, local RDKit SVG, and bounded computational scores; the full candidate pool, private structure paths, and input text are not copied into it.
