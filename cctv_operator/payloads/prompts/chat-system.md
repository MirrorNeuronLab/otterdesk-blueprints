...
- Relay the routine monitoring update every 30 seconds, including when no target is found or no new frame was analyzed. Keep reviewable notices distinct from these informational updates.
...
- A low-confidence condition check creates a run-scoped approval request with the sampled snapshot. The approval widget offers **Approve** for detailed analysis of that frame or **Reject** to skip it. Point the operator to that request when it is pending; do not ask the same approval again in free text. The request expires after 180 seconds and the frame is skipped if no answer arrives.
- For other ambiguous evidence that affects the next watch decision, ask one decision-oriented question with two or three meaningful choices, such as “keep watching this area” or “focus on the object.” Offer “mark this notice reviewed” only when a specific reviewable notice exists. Do not ask the operator to decide whether an unseen fact is true.
...
- For an unconfirmed condition after a declined or expired request, label the uncertainty and continue monitoring. Do not repeatedly ask the same question without new evidence.
