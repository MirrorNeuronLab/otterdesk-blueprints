# ROS 2 AMR Warehouse Demo

An operations-ready ROS 2 Jazzy simulation for an autonomous mobile robot
(AMR) in the AWS RoboMaker Small Warehouse. It combines Gazebo Harmonic,
[Nav2](https://docs.nav2.org/), a TurtleBot 3 model, ROS 2 video streaming, and
a LAN-accessible browser control room.

The warehouse dashboard shows an overhead camera plus the robot's RGB and
depth cameras. Operators can send short manual adjustment commands or request
autonomous Nav2 routes to the built-in A, B, and C warehouse zones.

## Quick start — warehouse demo

The demo is designed for a Linux host with Docker Compose and an NVIDIA GPU.
After cloning, build the overlay image once and start the warehouse target:

```bash
git clone git@github.com:homerquan/ros2-amr-demo.git
cd ros2-amr-demo
docker compose build overlay
docker compose up demo-world-warehouse
```

From any device on the same trusted LAN, open:

```text
http://<spark-lan-ip>:8088
```

The dashboard starts automatically with the warehouse world and provides:

| Capability | Details |
|---|---|
| Live video | Overhead, TurtleBot RGB, and TurtleBot depth camera streams |
| Autonomous AMR routes | **Move to A zone**, **Move to B zone**, and **Move to C zone** use Nav2 `NavigateToPose` goals |
| Route safety | **Cancel active route** stops the current navigation goal; manual controls also take over safely |
| Manual adjustment | Short Forward, Turn left, and Turn right commands with a ROS 2 watchdog stop |

The high-level route controls submit only fixed, map-validated warehouse
destinations. A ROS 2 gateway synchronizes AMCL with the simulator odometry,
then monitors and reports Nav2 route status back to the browser. This keeps the
LAN UI simple without exposing arbitrary navigation coordinates.

> The dashboard and ROS bridge are intended for a trusted LAN. Apply host
> firewall rules before exposing ports 8080, 8088, or 9090 beyond that network.

## Upstream credits and license

This repository is a derivative of the open-source
[pantelis/turtlebot-maze](https://github.com/pantelis/turtlebot-maze) project.
That project builds on the TurtleBot behavior-demo work originally created by
Sebastian Castro (2021–2024), with later enhancements by Pantelis Monogioudis
and contributors. Their attribution is retained here and in the project
history.

This fork keeps the upstream [MIT License](./LICENSE) unchanged, including the
original copyright notice. See `LICENSE` for the full terms.
...
| `demo-world-warehouse` | extends `overlay` | AWS RoboMaker Small Warehouse plus the video server and LAN dashboard |
| `warehouse-video-server` | extends `overlay` | ROS 2 image topics as MJPEG on port 8080 |
| `warehouse-video-ui` | `nginx:alpine` | Three-panel warehouse dashboard on port 8088 |
...
### Warehouse video dashboard

The warehouse command also starts an overhead camera, the TurtleBot's RGB and
depth camera streams, and a browser dashboard. Build the updated overlay once:
...
From any device on the same LAN, open `http://<spark-lan-ip>:8088`. The dashboard
uses these ROS 2 topics:
...
automatically started rosbridge WebSocket on port 9090 and publishes
`geometry_msgs/msg/TwistStamped` to `/web_cmd_vel`. A dedicated ROS 2 relay
converts this private browser topic to `/cmd_vel` and stops the robot whenever
commands stop arriving, including if the browser disconnects. Each button uses
a 0.5-second pulse. Because this control endpoint is LAN-accessible, use it
only on a trusted network.

The same panel also provides autonomous **Move to A zone**, **Move to B zone**,
and **Move to C zone** actions, plus **Cancel active route**. These request
only fixed, map-validated destinations from a ROS 2 navigation gateway; the
gateway sends a `nav2_msgs/action/NavigateToPose` goal to Nav2 and reports
route progress back to the dashboard. Choosing a manual control automatically
cancels an active route before issuing its velocity pulse.

For the Gazebo warehouse simulation, the gateway first synchronizes AMCL with
the simulated robot odometry before each route, so Nav2 plans from the AMR's
actual current position after any manual adjustment.

The raw MJPEG endpoint is also available on port 8080 (for example,
`http://<spark-lan-ip>:8080/stream?topic=/camera/color/image_raw`). Both ports
are intentionally LAN-accessible; restrict them with the host firewall if the
network is not trusted.

The warehouse target requests an NVIDIA GPU and uses surfaceless EGL, so
Gazebo's camera sensors also work when Spark is launched headlessly over SSH.
It runs Gazebo server-only and disables RViz because the browser dashboard is
its remote visual interface; other world targets retain their normal
interactive Gazebo/RViz behavior.
