...
Stress a portfolio against drawdown, rate shocks, and liquidity pressure before drafting possible rebalancing options.
...
Investors, advisors, portfolio managers, treasury teams, and fintech operators.
...
4. Ask the `Portfolio Risk Review Assistant` to observe the current state, compare valid actions, and choose among `rebalance_defensive`, `hedge_rates`, `raise_cash`.
...
```bash
cd portfolio_risk_review_assistant
