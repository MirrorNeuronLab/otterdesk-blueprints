...
The runtime projects identity, human-control policy, resources, agent roles, model defaults, and legal-playbook RAG settings from the manifest. The operator config contains only tunable values. The SDK resolves the staged input folder before workers run; the OCR and RAG skills select and prepare their own model runtimes.
