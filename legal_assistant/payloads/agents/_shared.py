...
from legal_domain import legacy
...
_spec = StatefulStepSpec(
    ...
    hooks=StepLifecycleHooks(append_event=legacy.append_event, runtime_step_mode="agent_invocation"),
)
...
def create_domain_agent(name: str, operation: Callable[..., dict[str, Any]]):
    def invoke(context: StatefulStepContext, *, agent_input: AgentInput, **options: Any) -> AgentHandlerOutput:
        ...
        ref = artifact_reference("legal_step_result", f"workflow_state/{name}_result.json")
        artifacts = [ref]
        payload: dict[str, Any] = {"step_id": name, "result_artifact": ref}
