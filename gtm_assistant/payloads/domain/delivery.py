...
DELIVERY_RECEIPT_PATH = "confidential_lifecycle_email_delivery_receipt.json"
...
def deliver_approved_development_email(
    ...
    *,
    ...
    """Send one explicitly approved, anonymized lifecycle-email rendering check."""
    ...
    if (smtp_host, smtp_port, security) != (ICLOUD_SMTP_HOST, ICLOUD_SMTP_PORT, "starttls"):
    ...
    draft = _first_approved_draft(interventions)
    request = {
    ...
    delivery_key = hashlib.sha256(
        "\x1f".join((approval_id, development_recipient.lower(), request["subject"], request["text"])).encode("utf-8")
    ).hexdigest()
    ...
    receipt = {
        "schema_version": "mn.gtm_assistant.email_delivery_receipt.v1",
    ...
    if result.get("status") != "sent":
        write_json(receipt_path, {**receipt, "status": "failed", "reason": str(result.get("reason") or "smtp_delivery_failed")})
...
def _first_approved_draft(interventions: Sequence[Mapping[str, Any]]) -> dict[str, str]:
    for intervention in interventions:
        if not isinstance(intervention, Mapping) or not bool((intervention.get("draft_review") or {}).get("approved")):
            continue
        draft = intervention.get("draft")
        if not isinstance(draft, Mapping):
            continue
        subject = str(draft.get("subject") or "").strip()
        body = _structured_draft_text(draft)
        if subject and body:
            return {"subject": subject, "body": body}
    raise RuntimeError("No quality-approved lifecycle draft is available")


def _structured_draft_text(draft: Mapping[str, Any]) -> str:
    parts = [str(draft.get("preview_text") or "").strip()]
    for section in draft.get("body_sections") or []:
        ...
        if title and body:
            ...
            parts.append(body)
    cta_label = str(draft.get("cta_label") or "").strip()
    if cta_label:
        parts.append(cta_label)
    footer = str(draft.get("footer") or "").strip()
    if footer:
        parts.append(footer)
...
def _development_subject(subject: str) -> str:
    return f"[Development test] {subject}"[:200]


def _development_body(body: str) -> str:
    return "Development delivery test only. No customer address or customer-specific data was used.\n\nHi there\n\n" + body
