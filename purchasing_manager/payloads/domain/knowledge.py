"""Purchase playbook loading and job-scoped retrieval mechanics."""
...
def load_purchase_knowledge(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "title": "Purchasing Manager Evidence And Review Playbook",
        "files": files,
...
def prepare_purchase_rag(config: dict[str, Any], root: Path, knowledge: dict[str, Any], documents: list[dict[str, Any]], run_id: str | None = None) -> dict[str, Any]:
    ...
    state: dict[str, Any] = {
        ...
        "config": raw,
        "namespace": f"{raw.get('namespace') or 'purchasing_manager'}:{run_id or 'local'}",
...
__all__ = ['load_purchase_knowledge', 'prepare_purchase_rag', 'retrieve_local_context', 'retrieve_purchase_rag_context', '_chunks']
