...
def normalize_inputs(inputs: dict[str, Any] | None) -> dict[str, Any]:
    ...
    aliases = {
        "vehicle": "car",
        "automobile": "car",
        "flight": "airline_ticket",
        "ticket": "airline_ticket",
        "rental": "rental_property",
        "ai_computer": "computer",
        "ai_workstation": "computer",
        "desktop": "computer",
...
_CONSTRAINT_KEY_ALIASES = {
    ...
    "minimum gpu vram": "min_gpu_vram_gb",
    "minimum gpu vram gb": "min_gpu_vram_gb",
    "minimum system ram": "min_system_ram_gb",
    "minimum system ram gb": "min_system_ram_gb",
    "minimum storage": "min_storage_tb",
    "minimum storage tb": "min_storage_tb",
    "minimum warranty years": "min_warranty_years",
    "local delivery required": "local_delivery_required",
    "local setup required": "local_setup_required",
    "local pickup required": "local_pickup_required",
    "shipping required": "shipping_required",
    "available for purchase required": "available_for_purchase_required",
    "maximum lead time business days": "max_lead_time_business_days",
}
...
def _add_constraint(constraints: dict[str, Any], value: str) -> None:
    ...
    constraints[key] = _coerce_constraint_value(key, raw_value.strip())


def _coerce_constraint_value(key: str, value: str) -> Any:
    if key == "zip_code":
        return value
    if key in {
        "min_gpu_vram_gb",
        ...
        return float(numeric.group(0)) if numeric else _coerce_scalar(value)
...
def _infer_purchase_type(description: str) -> str:
    ...
    if any(
        term in lowered
        for term in ("computer", "workstation", "desktop", "gpu", "graphics card")
    ):
        return "computer"
    return "custom"
