...
STREAM_PATH="${STREAM_PATH:-dam-access}"
...
SERVER_LOG="${SERVER_LOG:-/tmp/dam_access_watch_assistant_mediamtx.log}"
...
OPENSHELL_FRAME_BRIDGE_SOURCE_URI="${OPENSHELL_FRAME_BRIDGE_SOURCE_URI:-$SOURCE_URI}"
OPENSHELL_FRAME_BRIDGE_REMOTE_DIR="${OPENSHELL_FRAME_BRIDGE_REMOTE_DIR:-/sandbox/live}"
OPENSHELL_FRAME_BRIDGE_LOG="${OPENSHELL_FRAME_BRIDGE_LOG:-/tmp/dam_access_watch_assistant_openshell_frame_bridge.log}"
...
OPENSHELL_TUNNEL_LOG="${OPENSHELL_TUNNEL_LOG:-/tmp/dam_access_watch_assistant_openshell_rtsp_tunnel.log}"

usage() {
  cat <<EOF
Start a browser-playable local preview bridge for Dam Access Watch Assistant.

Default stream:
  ${STREAM_URI}

Requirements:
  - network access to the configured SOURCE_URI in rtsp mode
  - macOS camera permission for the terminal app running this script in browser or ffmpeg camera mode
...
Environment overrides:
  ...
  OPENSHELL_FRAME_BRIDGE_SOURCE_URI
                 Source used by the host-side frame bridge. Default: SOURCE_URI.
...
Examples:
  ...
  SOURCE_URI="rtsp://camera.example.com/live" scripts/start_webcam_stream_for_mac.sh
...
start_rtsp_server() {
  ...
  config_dir="$(mktemp -d /tmp/dam_access_watch_assistant_mediamtx.XXXXXX)"
...
EOF
  ...
  echo "Starting ${server_cmd} on rtsp://127.0.0.1:${RTSP_PORT}/${STREAM_PATH}"
...
start_openshell_rtsp_tunnel() {
  ...
  tunnel_config_dir="$(mktemp -d /tmp/dam_access_watch_assistant_openshell_tunnel.XXXXXX)"
...
grab_latest_frame() {
  ...
  ffmpeg \
    ...
    -nostdin \
    -rtsp_transport tcp \
    -i "$OPENSHELL_FRAME_BRIDGE_SOURCE_URI" \
    -frames:v 1 \
    -q:v 4 \
    -y \
...
start_openshell_frame_bridge() {
  ...
  frame_bridge_dir="$(mktemp -d /tmp/dam_access_watch_assistant_frames.XXXXXX)"
