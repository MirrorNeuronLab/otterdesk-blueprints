...
import { RemoteSource } from "./controls/remote.js";
...
async function boot({ scene, camera, renderer }) {
  ...
  const remoteSource = new RemoteSource({
    ...
    getNavigationState: () => compactMcpSnapshot({ includeRemoteBusy: false }),
    performBallAction: (action) => remoteBallAction(action),
  });
  // Keyboard last: it reads zero when idle, so it doubles as the fallback.
  const controller = new Controller({ sources: [remoteSource, padSource, touchSource, kbSource] });
  ...
  // The browser bridge can only use these intentionally narrow helpers. It
  // never reaches the debug window.rl surface or mutates MuJoCo state itself.
  function remoteBallAction(action) {
    if (inputLocked || locoSwitching || recovery) {
    ...
    if (loco !== "legs") return { accepted: false, reason: "kicks_require_legs" };
    const foot = action === "kick_left" ? "left" : action === "kick_right" ? "right" : "";
    if (!foot) return { accepted: false, reason: "unsupported_ball_action" };
    return triggerKick(foot, "remote")
      ? { accepted: true }
  ...
  function compactMcpSnapshot({ includeRemoteBusy = true } = {}) {
    const qpos = data.qpos;
    const qvel = data.qvel;
    const qw = qpos[3], qx = qpos[4], qy = qpos[5], qz = qpos[6];
    const yaw = Math.atan2(2 * (qw * qz + qx * qy), 1 - 2 * (qy * qy + qz * qz));
    const busy = !!(
      locoSwitching || recovery || rollRun || crouchRun || pickRun || kickRun ||
      postKickLock > 0 || sitTimer || standTimer || ceremony?.respawnActive ||
      (includeRemoteBusy && remoteSource.isActive())
    );
    const ballX = ballActive ? qpos[ballQposAdr] : null;
    const ballY = ballActive ? qpos[ballQposAdr + 1] : null;
    return {
      ready: !!(store().bootDone && !inputLocked && !busy),
      duck: {
        x: qpos[0],
        y: qpos[1],
        yaw,
      ...
      ball: {
        ...
        x: ballX,
        y: ballY,
        distance: ballActive ? Math.hypot(ballX - qpos[0], ballY - qpos[1]) : null,
      },
      active_command: remoteSource.status,
      recent_command: {},
    };
  }
  ...
  Object.assign(gameApi, {
    ...
    resetSim,
    spawnBall: () => spawnBall(),
    getMcpSnapshot: compactMcpSnapshot,
    startEntrance: () => ceremony.startEntrance(),
  });

  // ?control=1 keeps the visual simulator intact but bypasses the landing
  // click. The service still refuses motion until the entrance releases its
  // normal input lock and this tab holds the bridge lease.
  ...
  window.rl = {
    ...
    controller, kbSource, padSource, remoteSource,
