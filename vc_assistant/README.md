...
All actor-style LLM analysis prefers `nemotron3` through Docker Model Runner on a 48GB-or-above runtime node with GPU or integrated-GPU memory, including NVIDIA, Apple, AMD, and DGX Spark / GB10 unified-memory nodes reported by `mn status`. If no capable runtime node is available, the blueprint falls back to the runtime default model `gemma4:e2b`. Numerical formulas and missing-evidence gates remain deterministic.
