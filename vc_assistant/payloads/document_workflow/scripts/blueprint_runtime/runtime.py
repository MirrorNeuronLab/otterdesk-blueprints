...
def _source_workflow_steps() -> list[dict[str, Any]]:
    manifest_path = Path(__file__).resolve().parents[4] / "manifest.json"
...
WORKFLOW_STEP_IDS = [str(step["id"]) for step in _source_workflow_steps()]
RESEARCH_STAGE_IDS = [
...
def execute_runtime_handler(
    ...
    """Execute one already-resolved handler without consulting workflow topology."""

    step_id = str(step_id or "").strip()
    if not step_id:
        raise ValueError("VC Assistant workflow step id is required")
    ...
    try:
        result = handler(ctx, llm_client=llm_client)
        if debug_enabled:
            append_debug_record(
                ctx["run_dir"],
