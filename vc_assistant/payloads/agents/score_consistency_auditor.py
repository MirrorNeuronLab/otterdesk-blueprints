...
def run_score_consistency_auditor_step(ctx: dict[str, Any], *, llm_client: Any | None = None) -> dict[str, Any]:
    ...
    def audit_company(_context: dict[str, Any], item: dict[str, Any]) -> dict[str, Any]:
        ...
        methods = {
            method_id: score
            for method_id, score in store.read_entity_object("method_scores", company).items()
            if method_id in METHOD_IDS and isinstance(score, dict)
        }
        for method_id in METHOD_IDS:
            score = store.read_entity_object(f"method_scores_by_method/{method_id}", company)
            if score:
        ...
        analysis["research_reconciliation"] = store.read_entity_object("reconciliations", company) or reconcile_research(records, ledger)
