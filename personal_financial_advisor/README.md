...
This OtterDesk blueprint runs as a folder-monitoring local service made of collaborating actors: `financial_folder_watcher`, `financial_document_reader`, `financial_activity_classifier`, `financial_health_assessor`, `financial_market_researcher`, and `financial_advice_reporter`. The default demo is bounded with `monitoring.max_cycles=1`; set it to `null` only when intentionally running a continuous watcher. Each actor uses the configured Docker Model Runner `gemma4:e2b` profile with deterministic fallback evidence, while the shared `llm_ocr_skill` handles scanned or low-text pages.
...
For catalog launches, use a config override with `monitoring.max_cycles=null` when you want continuous polling instead of the bounded demo default.
...
- `config/default.json`: default OCR, bounded monitoring, output, public sample, and live model settings.
